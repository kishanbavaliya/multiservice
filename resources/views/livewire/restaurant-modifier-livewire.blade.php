<div class="p-6 bg-white rounded-lg shadow-lg">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Restaurant Modifiers</h2>
            <p class="text-gray-600 mt-1">Manage modifiers for restaurant products</p>
        </div>
    ...
    <div class="mb-6 space-y-4">
        ...
        <div class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <input wire:model.debounce.300ms="search" type="text" placeholder="Search modifiers..." 
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div>
                                ...
                                @if($modifier->description)
                                    <div class="text-sm text-gray-500">{{ Str::limit($modifier->description, 50) }}</div>
                                @endif
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $modifier->restaurant->name ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                {{ $modifier->status_color === 'green' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $modifier->status_text }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <button wire:click="edit({{ $modifier->id }})" class="text-blue-600 hover:text-blue-900">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </button>
                                
                                <button wire:click="toggleStatus({{ $modifier->id }})" class="text-green-600 hover:text-green-900">
                                    @if($modifier->status)
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button wire:click="delete({{ $modifier->id }})" 
                                        class="text-red-600 hover:text-red-900 {{ !$modifier->canBeDeleted() ? 'opacity-50 cursor-not-allowed' : '' }}"
                                        {{ !$modifier->canBeDeleted() ? 'disabled' : '' }}
                                        title="{{ !$modifier->canBeDeleted() ? 'Cannot delete - in use by products' : 'Delete modifier' }}">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                            No modifiers found. {{ $search ? 'Try adjusting your search criteria.' : 'Start by adding your first modifier.' }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <!-- Pagination -->
    @if(method_exists($modifiers, 'hasPages') && $modifiers->hasPages())
        <div class="mt-6">
            {{ $modifiers->links() }}
        </div>
    @endif
        ...
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" wire:click="closeModal">
            <div class="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white" wire:click.stop>
                ...
                <form wire:submit.prevent="save" class="space-y-6">
                    ...
                    <!-- Name -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Modifier Name <span class="text-red-500">*</span></label>
                        <input wire:model="name" type="text" placeholder="e.g., Extra Cheese, No Onions, Spicy Level" 
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="3" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                  placeholder="Optional description for this modifier"></textarea>
                    ...
                    <div class="flex justify-end space-x-3 pt-4 border-t">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                            {{ $isEditing ? 'Update Modifier' : 'Create Modifier' }}
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mt-4">Delete Modifier</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-500">Are you sure you want to delete this modifier? This action cannot be undone.</p>
                    </div>
